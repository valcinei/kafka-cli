...
module.exports = {
  ...
  run: async (toolbox: GluegunToolbox) => {
    const aksHost = {
      type: 'input',
      name: 'host',
      message: 'Whats host you broker?(default:localhost:9092)'
    }
    const aksTopic = {
      type: 'input',
      name: 'topics',
      message: 'Whats topic you want consume?(separated by comma)'
    }
    const aksPartition = {
      type: 'input',
      name: 'partition',
      message: 'How many partitions?(default: 0)'
    }

    const { host, topics, partitions } = await toolbox.prompt.ask([
      aksHost,
      aksTopic,
      aksPartition
    ])
...
const startConsumer = async (
  topicList?: Array<OffsetFetchRequest>,
